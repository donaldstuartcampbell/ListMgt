...
Module M_ListMgt_Files
    ...
    End Structure
    Structure BirthdayRecordType '94 + 5 * 4 =20 + 2 * 8 = 16 or 130
...
#Region "NewRecord"
    ...
    Function getAll_ApptTodoRecordType_Read1by1() As ApptTodoRecordType() '2.3 secs for read 102600 recs
        Dim info As New FileInfo(xBuildFullFileName(file_ListMgt.FileName))
        Dim nRecs As Integer = info.Length / file_ListMgt.FileLen
...
#End Region
    ...
    Function getBirthdaysforGivenMonth(ByVal mth As Integer)
        ...
        Return a
    End Function
    Public Class ApptGV 'ApptGlobalVariabeles
        Public Shared ContractorID As Integer = 0
        Public Shared TemplateDir As String = "C:\SealcoatCorp\Templates\"
        Public Shared NewProposal As ContractorProposalsType
        Public Shared CusID_NewQuote As Integer


        Public Shared path_scQuotes As String = "C:\SealcoatCorp\sc\word\Quotes\"
        Public Shared path_scQfiles As String = "C:\SealcoatCorp\Qfiles\"
        Public Shared PDFfileName As String
        Public Shared wordFullFileName As String 'used when calling F_showWordDoc


        'the following added 10/16/2018
        Public Shared QuoteAcctNum As Integer

        '==


        Public Shared SQLdbName As String = "SealcoatCorp2"

        'Public Shared SQLconnString As String = "Data Source=DESKTOP-4KSC1BA\SQLEXPRESS;Database = TodayDB;Integrated Security=True;Connect Timeout=15;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False"
        'Public Shared SQLconnString As String = "Data Source=DESKTOP-B9HJIN3\SQLEXPRESS;Database = SealcoatCorp;Integrated Security=True;Connect Timeout=15;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False"

        'changed the following on 8/28/2-18
        'Public Shared SQLconnString As String = "Data Source=DESKTOP-4KSC1BA\SQLEXPRESS;Database = TodayDB;Integrated Security=True;Connect Timeout=15;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False"

        'Public Shared SQLconnString As String = "Data Source=DESKTOP-4KSC1BA\SQLEXPRESS;Database = SealcoatCorp2;Integrated Security=True;Connect Timeout=15;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False"
        'Public Shared SQLconnStringSC As String = "Data Source=DESKTOP-4KSC1BA\SQLEXPRESS;Database = SealcoatCorp2;Integrated Security=True;Connect Timeout=15;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False"
        'Public Shared SQLconnStringTODO As String = "Data Source=DESKTOP-4KSC1BA\SQLEXPRESS;Database = TODO;Integrated Security=True;Connect Timeout=15;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False"

        Public Shared SQLconnString As String = "Data Source=DESKTOP-4KSC1BA\SQLEXPRESS;Database = TODO;Integrated Security=True;Connect Timeout=15;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False"


        Public Shared DBbeingUsed As String = "TODO" ' "SC"

        'DESKTOP-B9HJIN3\SQLEXPRESS
        'SealcoatCorp
        Public Shared CusID As Integer
        Public Shared AddEditContactID As Integer
        Public Shared UserID As Integer

#Region "standalone"
        'Public Shared ccPath As String = "C:\todoProgramCOM2020" 'c:\todo\" '"c:\testFiles\"
        ...
        Public Shared resetAllApptArray As Boolean = True
        Public Shared refresh_pbCalGrid As Boolean = False

        Public Shared AllApptArrayCount As Integer = 0
        Public Shared AllAppts_LongInteger(0) As LongIntegerType

        Public Shared iMeanAll(0) As LongIntegerType 'New as of 11/11/2015 (no filtering at all - ie testing for blanks and delete)
        'above is useful when adding new appt records and testing whether or not a record exist

        Public Shared SelectedDate_ApptRecords(0) As ApptRecordsType

        Public Shared GridStartHour As Integer = 9 '7 '7am
        'Public Shared GridEndHour As Integer = 18  '6pm 12+6
        Public Shared GridEndHour As Integer = 18 ' 23 '18 '21  '9pm 12+6

        Public Shared GridNum_TextChanged As Integer
        Public Shared GridText_TextChanged As String = ""
        Public Shared GridMinute_TextChanged As Integer
        Public Shared GridDate_TextChanged As Date

        Public Shared nItemInGrid As Integer
        Public Shared tbAppt() As TextBox
        Public Shared lblAppt() As Label
        ' Public Shared lblFill() As Label

        Public Shared checkBoxAppt() As CheckBox 'new 20150106 -dsc

        Public Shared TodaysDate As Date = Date.Today
        Public Shared SelectedDate As Date = Date.Today

        'Public Shared SelectedDiaryDate As Date = Today

        Public Shared MinInDay(1439) As TwoIntegersType

        Public Shared SecInDay(86399) As TwoIntegersType
        Public Shared tm(0) As timeMsgType

        ''''Public Shared ApptForDay(1439) As ApptRecType '1440 minutes in day

        Public Shared BDayRecs(0) As StringPointerType
        Public Shared BDayRecsJan1(0) As StringPointerType
        Public Shared BDayRecsToday(0) As StringPointerType
        Public Shared BDayUpcomingRecs(0) As StringPointerType
        Public Shared LowBDayTics As Long = 946080000000000 ' 1/1/0004
        Public Shared HighBDayTics As Long = 1261440000000000 ' 12/31,0004
        Public Shared FirstDayAfterBDayTics As Long = 1262304000000000 '860,000,000,000 add to HighBDayTics (seconds in day - 10,000,000 = 1 second)
        Public Shared March1Tics As Long = 997920000000000 '#3/1/0004#  '?new datetime(4,3,1).Ticks '997920000000000
        Public Shared ResetUpcoming_Flag As Boolean = True
        Public Shared HourGridControl As Integer = -1 'contains an hour from 0-23 which will expand the grid for that hour to 15 minute slots
        'only one hour will work. When clicked a second time will collapse the grid back to one hour or if a different hour is selected


        Public Shared BDayRecsArray(0) As LongIntegerType 'where Long=Tics and Integer=AcctNum - sort by AcctNum
        Public Shared BDayRecsArray_AcctNum As Integer = 0 ' hold the actual acctNum when name is highlighted and there was a hit - else this holds a value of 0

        Public Shared NumberOfTextBoxesToCreate As Integer = 80 ' was 40'20140821
        'Public Shared LenApptRecord As Integer = 100 'added '20140821 


        Public Shared TodayDiary As String


        ' Public Shared p As New Printer 'Imports Microsoft.VisualBasic.PowerPacks.Printing.Compatibility.VB6 allows this to work
        Public Shared UsingPDF As Boolean = False

        'Public Shared appt_fullFileName As String = xBuildFullFileName( f_Appt.filename
        'xBuildFullFileName(f_Appt.filename)

        '0 means add Record else Update record

        'new as of 4/1/2015
        Public Shared nBdayRecs As Integer
        Public Shared nApptRecs As Integer
        Public Shared nBdayApptRecs As Integer
        Public Shared positionOfToDoBlank As Integer
        Public Shared PanelApptWidth As Single 'to be set a beg of program

        Public Shared TodoGridOnly As Boolean 'used when modifying and updating todo (in todoGridOnly)

        Public Shared InclRecur As Boolean = False ' True
        Public Shared RecurRecordsForSelectedDate() As ApptRecordsType
        Public Shared AllRecurRecords(0) As ApptRecordsType 'added 6/28/2017 in mBday 'important includes Location in File
        Public Shared LongType(0) As Long


        Public Shared Use_ApptTodoRecords() As ApptTodoRecordType
        Public Shared All_ApptTodoRecords() As ApptTodoRecordType
        Public Shared All_ApptTodoRecords_Cnt As Integer
        Public Shared distinctDates() As Long
        Public Shared TodoDistinctDates() As Long
        Public Shared todoDistinctDatesStr() As String
        ...
        Public Shared UsingApptTodoSystem As Boolean = True 'make false if using SQL '10/14/2019

        Public Shared MaxApptTodos As Integer = 200
        'Note: if true then change startup form to F_ApptTodo from F_Todo

        Public Shared Todo_SelDate As Date
        Public Shared TodoDeletes_SelDate As Date
        Public Shared Appt_SelDate As Date

        Public Shared Highest_dTicsOriginal As Long
        'Public Shared useFlashDrive As Boolean = False
        Public Shared CreateDate_LOCATIONarray() As Integer
        Public Shared Holidays() As HolidaysType
        Public Shared HolidayDates() As Date

        Public Shared Birthdays() As BirthdayRecordType
        Public Shared BirthdayDates() As Date

#End Region'standalone
    End Class
End Module
